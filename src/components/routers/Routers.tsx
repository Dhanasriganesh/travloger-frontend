import React from 'react'
import { Routes, Route, Navigate } from 'react-router-dom'
import AdminDashboard from '../pages/AdminDashboard'
import ProtectedRoute from '../auth/ProtectedRoute'
import Login from '../../pages/Login'
import Leads from '../pages/crm/Leads'
import Bookings from '../pages/crm/Bookings'
import Payments from '../pages/crm/Payments'
import Employees from '../pages/Employees'
import MainWebsiteEdit from '../pages/cms/MainWebsiteEdit'
import WebsiteEdit from '../pages/cms/WebsiteEdit'
import Queries from '../pages/Queries'
import QueryDetail from '../pages/QueryDetail'
import GuestDocuments from '../pages/GuestDocuments'
import Settings from '../pages/Settings'
import Activities from '../pages/cms/Activities'
import AuditLogMaster from '../pages/cms/AuditLogMaster'
import DayItineraryMaster from '../pages/cms/DayItineraryMaster'
import Destinations from '../pages/cms/Destinations'
import ExpenseTracking from '../pages/cms/ExpenseTracking'
import Hotels from '../pages/cms/Hotels'
import HotelRates from '../pages/cms/HotelRates'
import Itineraries from "../pages/cms/Itineraries"
import IntegrationSettingsMaster from '../pages/cms/IntegrationSettingsMaster'
import ItineraryNotesInclusions from '../pages/cms/ItineraryNotesInclusions'
import ItineraryBuilder from '../pages/cms/ItineraryBuilder'
import ItineraryTemplate from '../pages/cms/ItineraryTemplate'
import PackageDetails from '../pages/cms/PackageDetails'
import LeadScoringMaster from '../pages/cms/LeadScoringMaster'
import LeadTypeMaster from '../pages/cms/LeadTypeMaster'
import MealPlans from '../pages/cms/MealPlans'
import MediaLibrary from '../pages/cms/MediaLibrary'
import NotificationTemplateMaster from '../pages/cms/NotificationTemplateMaster'
import PackageTheme from '../pages/cms/PackageTheme'
import Policy from '../pages/cms/Policy'
import PricingTaxRule from '../pages/cms/PricingTaxRule'
import ProfitCalculationMaster from '../pages/cms/ProfitCalculationMaster'
import QueryStatus from '../pages/cms/QueryStatus'
import RoomTypes from '../pages/cms/RoomTypes'
import States from '../pages/cms/States'
import Suppliers from '../pages/cms/Suppliers'
import Transfers from '../pages/cms/Transfers'
import UserRoleAccess from '../pages/cms/UserRoleAccess'
import UtmTrackingSetup from '../pages/cms/UtmTrackingSetup'
import VehicleDriver from '../pages/cms/VehicleDriver'
import VehiclePackageRates from '../pages/cms/VehiclePackageRates'
import VehicleTypes from '../pages/cms/VehicleTypes'
import VendorPayoutMaster from '../pages/cms/VendorPayoutMaster'
import Reports from '../pages/crm/Reports'

const Routers: React.FC = () => {
  return (
    <Routes>
      <Route path="/login" element={<Login />} />
      <Route
        path="/"
        element={
          <ProtectedRoute>
            <AdminDashboard />
          </ProtectedRoute>
        }
      />
      <Route
        path="/leads"
        element={
          <ProtectedRoute>
            <Leads />
          </ProtectedRoute>
        }
      />
      <Route
        path="/bookings"
        element={
          <ProtectedRoute>
            <Bookings />
          </ProtectedRoute>
        }
      />
      <Route
        path="/employees"
        element={
          <ProtectedRoute>
            <Employees />
          </ProtectedRoute>
        }
      />
      <Route
        path="/payments"
        element={
          <ProtectedRoute>
            <Payments />
          </ProtectedRoute>
        }
      />
      <Route
        path="/packages"
        element={
          <ProtectedRoute>
            <Itineraries />
          </ProtectedRoute>
        }
      />
      <Route
        path="/packages/:id"
        element={
          <ProtectedRoute>
            <PackageDetails />
          </ProtectedRoute>
        }
      />
      <Route
        path="/packages/:id/edit"
        element={
          <ProtectedRoute>
            <ItineraryBuilder />
          </ProtectedRoute>
        }
      />
      <Route
        path="/reports"
        element={
          <ProtectedRoute>
            <Reports />
          </ProtectedRoute>
        }
      />
      <Route
        path="/main-website-edit"
        element={
          <ProtectedRoute>
            <MainWebsiteEdit />
          </ProtectedRoute>
        }
      />
      <Route
        path="/website-edit"
        element={
          <ProtectedRoute>
            <WebsiteEdit />
          </ProtectedRoute>
        }
      />
      <Route
        path="/queries"
        element={
          <ProtectedRoute>
            <Queries />
          </ProtectedRoute>
        }
      />
      <Route
        path="/queries/:id"
        element={
          <ProtectedRoute>
            <QueryDetail />
          </ProtectedRoute>
        }
      />
      <Route
        path="/guest-documents"
        element={
          <ProtectedRoute>
            <GuestDocuments />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings"
        element={
          <ProtectedRoute>
            <Settings />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/admin"
        element={
          <ProtectedRoute>
            <Settings />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/activity"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <Activities />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/audit-log-master"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <AuditLogMaster />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/day-itinerary"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <DayItineraryMaster />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/destinations"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <Destinations />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/expense-tracking-master"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <ExpenseTracking />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/hotels"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <Hotels />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/hotel-rates"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <HotelRates />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/integration-settings-master"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <IntegrationSettingsMaster />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/itinerary-notes-inclusions"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <ItineraryNotesInclusions />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/itinerary-template"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <ItineraryTemplate />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/lead-scoring-master"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <LeadScoringMaster />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/lead-type-master"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <LeadTypeMaster />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/meal-plan"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <MealPlans />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/media-library"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <MediaLibrary />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/notification-template-master"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <NotificationTemplateMaster />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/package-theme"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <PackageTheme />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/policy"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <Policy />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/pricing-tax-rule"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <PricingTaxRule />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/profit-calculation-master"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <ProfitCalculationMaster />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/query-status"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <QueryStatus />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/room-types"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <RoomTypes />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/states"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <States />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/suppliers"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <Suppliers />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/transfers"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <Transfers />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/user-role-access"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <UserRoleAccess />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/utm-tracking-setup"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <UtmTrackingSetup />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/vehicle-driver"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <VehicleDriver />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/vehicle-package-rates"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <VehiclePackageRates />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/vehicle-types"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <VehicleTypes />
          </ProtectedRoute>
        }
      />
      <Route
        path="/settings/vendor-payout-master"
        element={
          <ProtectedRoute requiredRole="Super Admin">
            <VendorPayoutMaster />
          </ProtectedRoute>
        }
      />
      <Route path="*" element={<Navigate to="/" replace />} />
    </Routes>
  )
}

export default Routers

