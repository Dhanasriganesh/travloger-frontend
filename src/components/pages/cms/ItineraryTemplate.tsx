...
const ItineraryTemplate: React.FC = () => {
  ...
  const fetchMasters = async () => {
    try {
      // Fetch available masters with error handling for each
      const fetchMasterData = async (endpoint: string, fallback: any = []) => {
        try {
          const data = await fetchApi(endpoint)
          return data
        } catch (error) {
          console.warn(`Failed to fetch ${endpoint}:`, handleApiError(error))
          return fallback
        }
      }
      ...
      const [
        ...
        fetchMasterData('/api/destinations', { destinations: [] }),
        fetchMasterData('/api/day-itineraries', { dayItineraries: [] }),
        fetchMasterData('/api/package-themes', { packageThemes: [] }),
        fetchMasterData('/api/policies', { policies: [] }),
        fetchMasterData('/api/hotels', { hotels: [] }),
        fetchMasterData('/api/transfers', { transfers: [] }),
        fetchMasterData('/api/activities', { activities: [] }),
      ...
      if (destinationNames.length === 0) {
        ...
        setDestinations(['Delhi', 'Mumbai', 'Bangalore', 'Chennai', 'Kolkata', 'Hyderabad', 'Pune', 'Ahmedabad', 'Jaipur', 'Goa', 'Kerala', 'Agra', 'Varanasi', 'Rishikesh', 'Manali', 'Shimla'])
      } else {
        setDestinations(destinationNames)
      }

      // Set day itineraries with fallback data
      const dayIts = dayItData.dayItineraries || []
      if (dayIts.length === 0) {
        // Fallback day itineraries if API is not available
        setDayItineraries([
          { id: 1, name: 'Sample Day Itinerary 1', destinations: ['Delhi'], numDays: 1 },
          { id: 2, name: 'Sample Day Itinerary 2', destinations: ['Mumbai'], numDays: 2 },
          { id: 3, name: 'Sample Day Itinerary 3', destinations: ['Goa'], numDays: 3 }
        ])
      } else {
        setDayItineraries(dayIts)
      }
  ...
  const fetchTemplates = async () => {
    try {
      setLoading(true)
      const data = await fetchApi('/api/itinerary-templates')
      setTemplates(data.templates || [])
    } catch (error) {
      console.error('Error fetching templates:', handleApiError(error))
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSaveTemplate = async () => {
    ...
    try {
      ...
      console.error('Error saving template:', handleApiError(error))
      alert(`Error saving template: ${handleApiError(error)}`)
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleDeleteTemplate = async (id: number, name: string) => {
    ...
    try {
      const data = await fetchApi(`/api/itinerary-templates?id=${id}`, {
        method: 'DELETE'
      })
      ...
      console.error('Error deleting template:', handleApiError(error))
      alert(`Error deleting template: ${handleApiError(error)}`)
    }
  }
