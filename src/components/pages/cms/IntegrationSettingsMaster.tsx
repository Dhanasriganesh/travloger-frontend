...
const IntegrationSettingsMaster: React.FC = () => {
  ...
  const fetchSettings = async () => {
    try {
      ...
      const data = await fetchApi(`/api/integration-settings?${params.toString()}`)
      setSettings(data.settings || [])
    } catch (error) {
      console.error('Error fetching integration settings:', handleApiError(error))
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const data = await fetchApi('/api/integration-settings', {
        method,
      ...
      if (data.success) {
        ...
        alert(`Failed to save integration: ${data.details || data.error}`)
      }
    } catch (error) {
      console.error('Error saving integration:', handleApiError(error))
      alert(`Failed to save integration: ${handleApiError(error)}`)
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleDelete = async (id: number, name: string) => {
    ...
    try {
      const data = await fetchApi(`/api/integration-settings?id=${id}`, {
        method: 'DELETE'
      })
      ...
      console.error('Error deleting integration:', handleApiError(error))
      alert(`Failed to delete integration: ${handleApiError(error)}`)
    }
  }
