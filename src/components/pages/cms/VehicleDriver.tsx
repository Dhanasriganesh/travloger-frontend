...
const VehicleDriver: React.FC = () => {
  ...
  const fetchMasters = async () => {
    // Fetch vehicle types with fallback
    try {
      const vehicleTypesData = await fetchApi('/api/vehicle-types')
      setVehicleTypes(vehicleTypesData.vehicleTypes || [])
    } catch (error) {
      console.warn('Vehicle types API not available, using fallback data')
      setVehicleTypes([
        { id: 1, vehicle_type: 'Sedan' },
        { id: 2, vehicle_type: 'SUV' },
        { id: 3, vehicle_type: 'Hatchback' },
        { id: 4, vehicle_type: 'Bus' },
        { id: 5, vehicle_type: 'Tempo Traveller' }
      ])
    }

    // Fetch suppliers with fallback
    try {
      const suppliersData = await fetchApi('/api/suppliers')
      setSuppliers(suppliersData.suppliers || [])
    } catch (error) {
      console.warn('Suppliers API not available, using fallback data')
      setSuppliers([
        { id: 1, company_name: 'Local Transport Services' },
        { id: 2, company_name: 'City Cab Services' },
        { id: 3, company_name: 'Travel Partners' }
      ])
    }
  }
  ...
  const fetchVehicleDrivers = async () => {
    try {
      setLoading(true)
      const data = await fetchApi('/api/vehicle-drivers')
      setVehicles(data.vehicleDrivers || [])
    } catch (error) {
      console.error('Error fetching vehicle drivers:', handleApiError(error))
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleDeleteVehicle = async (id: number, name: string) => {
    ...
    try {
      const data = await fetchApi(`/api/vehicle-drivers?id=${id}`, {
        method: 'DELETE'
      })
        ...
        <div className="max-w-7xl mx-auto px-4 py-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-lg font-bold text-gray-900">Vehicle & Driver Master</h1>
