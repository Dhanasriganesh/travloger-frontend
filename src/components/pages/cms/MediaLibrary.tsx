...
const MediaLibrary: React.FC = () => {
  ...
  const fetchMediaItems = async () => {
    try {
      setLoading(true)
      const data = await fetchApi('/api/media-library')
      setMediaItems(data.mediaItems || [])
    } catch (error) {
      console.error('Error fetching media items:', handleApiError(error))
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSaveMedia = async () => {
    ...
    try {
      ...
      console.error('Error saving media item:', handleApiError(error))
  ...
  const handleDeleteMedia = async (id: number, name: string) => {
    ...
    try {
      const data = await fetchApi(`/api/media-library?id=${id}`, {
        method: 'DELETE'
      })
      ...
      console.error('Error deleting media item:', handleApiError(error))
