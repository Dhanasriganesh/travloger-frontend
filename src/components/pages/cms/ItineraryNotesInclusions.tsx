...
const ItineraryNotesInclusions: React.FC = () => {
  ...
  const fetchNotesInclusions = async () => {
    try {
      setLoading(true)
      const data = await fetchApi('/api/itinerary-notes-inclusions')
      setNotesInclusions(data.notesInclusions || [])
    } catch (error) {
      console.error('Error fetching notes/inclusions:', handleApiError(error))
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSaveNoteInclusion = async () => {
    ...
    try {
      ...
      console.error('Error saving note/inclusion:', handleApiError(error))
      alert(`Error saving note/inclusion: ${handleApiError(error)}`)
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleDeleteNoteInclusion = async (id: number, name: string) => {
    ...
    try {
      const data = await fetchApi(`/api/itinerary-notes-inclusions?id=${id}`, {
        method: 'DELETE'
      })
      ...
      console.error('Error deleting note/inclusion:', handleApiError(error))
      alert(`Error deleting note/inclusion: ${handleApiError(error)}`)
    }
  }
