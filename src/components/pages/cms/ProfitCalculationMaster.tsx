...
const ProfitCalculationMaster: React.FC = () => {
  ...
  const fetchCalculations = async () => {
    try {
      ...
      console.error('Error fetching profit calculations:', handleApiError(error))
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await fetchApi('/api/profit-calculation', {
        method,
      ...
      resetForm()
      alert(data.message || 'Profit calculation saved successfully')
    } catch (error) {
  ...
  const handleRecalculate = async (calculation: ProfitCalculation) => {
    try {
      ...
      await fetchCalculations()
      alert(data.message || 'Profit calculation updated successfully')
    } catch (error) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const data = await fetchApi(`/api/profit-calculation?id=${id}`, {
        method: 'DELETE'
      })

      await fetchCalculations()
      alert(data.message || 'Profit calculation deleted successfully')
    } catch (error) {
