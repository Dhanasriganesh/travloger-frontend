...
function MainWebsiteEdit() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        setLoading(true)
        const data = await fetchApi<{ hero?: any; sections?: any }>('/api/cms/site/home')
        if (data?.hero) {
          setHomeHero({
  ...
  const saveHomeHero = async (): Promise<void> => {
    try {
      setSaving(true)
      setError(null)
      await fetchApi('/api/cms/site/home', {
        method: 'PUT',
  ...
  const saveOverview = async (): Promise<void> => {
    try {
      ...
      const currentData = await fetchApi<{ sections?: any }>('/api/cms/site/home')
