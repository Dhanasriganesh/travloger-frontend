...
const PackageTheme: React.FC = () => {
  ...
  const fetchMasters = async () => {
    try {
      const [dayItRes, leadTypesRes] = await Promise.all([
        fetchApi('/api/day-itineraries').catch(() => null),
        fetchApi('/api/lead-types').catch(() => null) // Lead Type might not exist yet
      ])
      
      if (dayItRes) {
        try {
          setDayItineraries((dayItRes.dayItineraries || []).map((it: any) => ({
            id: it.id,
      ...
      if (leadTypesRes) {
        try {
          setLeadTypes(leadTypesRes.leadTypes || [])
        } catch (error) {
  ...
  const fetchPackageThemes = async () => {
    try {
      setLoading(true)
      const data = await fetchApi('/api/package-themes')
      ...
      console.error('Error fetching package themes:', handleApiError(error))
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSaveTheme = async () => {
    ...
    try {
      ...
      console.error('Error saving package theme:', handleApiError(error))
  ...
  const handleDeleteTheme = async (id: number, name: string) => {
    ...
    try {
      const data = await fetchApi(`/api/package-themes?id=${id}`, {
        method: 'DELETE'
      })
      ...
      console.error('Error deleting package theme:', handleApiError(error))
