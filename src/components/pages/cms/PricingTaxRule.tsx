...
const PricingTaxRule: React.FC = () => {
  ...
  const fetchPricingRules = async () => {
    try {
      setLoading(true)
      const data = await fetchApi('/api/pricing-tax-rules')
      setPricingRules(data.pricingRules || [])
    } catch (error) {
      console.error('Error fetching pricing rules:', handleApiError(error))
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleDeleteRule = async (id: number, name: string) => {
    ...
    try {
      const data = await fetchApi(`/api/pricing-tax-rules?id=${id}`, {
        method: 'DELETE'
      })
