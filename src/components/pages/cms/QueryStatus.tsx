...
const QueryStatus: React.FC = () => {
  ...
  const fetchQueryStatuses = async () => {
    try {
      setLoading(true)
      const data = await fetchApi('/api/query-statuses')
      ...
      console.error('Error fetching query statuses:', handleApiError(error))
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleDeleteStatus = async (id: number, name: string) => {
    ...
    try {
      const data = await fetchApi(`/api/query-statuses?id=${id}`, {
        method: 'DELETE'
      })
                    ...
                    {filteredStatuses.length === 0 ? (
                      <tr>
                        <td colSpan={9} className="px-3 py-4 text-center text-sm text-gray-500">
                          {searchTerm ? 'No query statuses found matching your search' : 'No query statuses found. Click "Add Status" to create one.'}
                        </td>
                      </tr>
                    ) : (
