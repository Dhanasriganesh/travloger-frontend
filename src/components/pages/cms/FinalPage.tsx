  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const [daysData, eventsData, hotelsData, transfersData] = await Promise.all([
          fetchApi(`/api/itineraries/${itinerary.id}/days`),
          fetchApi(`/api/itineraries/${itinerary.id}/events`),
          fetchApi('/api/hotels'),
          ...
          <div className="max-w-4xl mx-auto px-6 py-4">
            ...
            <div className="mt-4 flex justify-center">
              <button
                onClick={exportToPDF}
                disabled={exporting}
                className={`px-6 py-3 rounded-lg font-semibold text-sm transition-all duration-200 flex items-center gap-2 ${exporting
