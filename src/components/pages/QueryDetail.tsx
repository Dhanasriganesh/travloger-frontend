...
const QueryDetail: React.FC<QueryDetailProps> = ({ queryId: propQueryId, onBack, isEmployee = false }) => {
  ...
  useEffect(() => {
    const fetchQuery = async () => {
      try {
        ...
        const data = await fetchApi<{ lead?: any }>(`/api/leads/${queryId}`)
  ...
  const calculateTotalPrice = useCallback(async (itineraryId: number) => {
    try {
      const data = await fetchApi<{ events?: any[] }>(`/api/itineraries/${itineraryId}/events`)
  ...
  const fetchPackageSuggestions = useCallback(async () => {
    try {
      ...
      const data = await fetchApi<{ itineraries?: any[] }>('/api/itineraries', { cache: 'no-store' })
      
      const itineraries = Array.isArray(data.itineraries) ? data.itineraries : []
      console.log('🔍 Total itineraries fetched:', itineraries.length)
      console.log('🎯 Query destination:', query?.destination)
      console.log('📋 All itineraries:', itineraries.map((i: any) => ({ id: i.id, name: i.name, destinations: i.destinations })))
      ...
      if (suggestionsWithPrices.length === 0 && itineraries.length > 0) {
        console.log('⚠️ No matching suggestions, showing all itineraries for debugging')
        const allSuggestions = await Promise.all(
          itineraries.slice(0, 5).map(async (itinerary: any) => {
            const totalPrice = await calculateTotalPrice(itinerary.id)
            return {
              id: itinerary.id,
              title: itinerary.name || 'Untitled Package',
  ...
  const handleBackToQueries = () => {
    if (onBack) {
      ...
      navigate(isEmployee ? '/employee' : '/queries')
    }
  }
  ...
  const saveClientChanges = async () => {
    ...
    try {
      ...
      await fetchApi(`/api/leads/${queryId}`, {
        method: 'PATCH',
        body: JSON.stringify({
      ...
      const message = handleApiError(error, 'Failed to update client information. Please try again.')
  ...
  const saveQueryChanges = async () => {
    ...
    try {
      ...
      const message = handleApiError(error, 'Failed to update query information. Please try again.')
                ...
                <div className="flex items-center gap-2">
                  <Button 
                    ...
                    onClick={() => {
                      ...
                      navigate(`/queries/${queryId}${isEmployee ? '?view=employee' : ''}`)
                    }}
                  ...
                  <Button 
                    ...
                    onClick={() => {
                      ...
                      navigate(`/queries/${queryId}/proposals${isEmployee ? '?view=employee' : ''}`)
                    }}
                  ...
                  <Button 
                    ...
                    onClick={() => {
                      ...
                      navigate(`/queries/${queryId}/billing${isEmployee ? '?view=employee' : ''}`)
                    }}
