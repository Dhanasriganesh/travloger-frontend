...
const Billing: React.FC<BillingProps> = ({ queryId, queryData }) => {
  ...
  const fetchPayments = useCallback(async () => {
    try {
      setLoading(true)
      const data = await fetchApi<{ payments?: Payment[] }>(`/api/query-payments?queryId=${queryId}`)
  ...
  const fetchConfirmedProposals = useCallback(async () => {
    try {
      ...
      const data = await fetchApi<{ confirmedProposals?: ConfirmedProposal[]; totalAmount?: number }>(
        `/api/confirmed-proposals?queryId=${queryId}`
      )
  ...
  const handleDeletePayment = async (id: number) => {
    ...
    try {
      await fetchApi(`/api/query-payments?id=${id}`, { method: 'DELETE' })
      fetchPayments()
    } catch (error) {
                ...
                onClick={async () => {
                  try {
                    ...
                    if (planStatus === 'Paid' && planReceipt) {
                      ...
                      try {
                        const upJson = await fetchApi<{ url: string }>('/api/upload', { method: 'POST', body: fd })
                        receiptUrl = upJson.url
                      } catch (uploadError) {
                        alert(handleApiError(uploadError, 'Failed to upload receipt'))
                        return
                      }
                    }
                    ...
                    await fetchApi('/api/query-payments', {
                    ...
                    console.error(e)
                    alert(handleApiError(e, 'Failed to save payment plan'))
                  }
                }}
