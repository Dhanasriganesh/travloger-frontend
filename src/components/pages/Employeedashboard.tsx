...
const Employeedashboard: React.FC = () => {
  ...
  // Helper function to safely extract destination name from string or object
  const getDestinationName = (destination: any): string => {
    if (!destination) return ''
    if (typeof destination === 'string') return destination
    if (typeof destination === 'object') {
      return destination.name || destination.destination || ''
    }
    ...
    try {
      ...
      const paymentLinkRes = await fetch('/api/razorpay/create-payment-link', {
        ...
        body: JSON.stringify({
          ...
          description: `Payment for ${getDestinationName(selectedItinerary.destination)} travel package`,
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <h2 className="text-lg font-semibold text-gray-900">
                          {getDestinationName(selectedItinerary.destination)} assigning to {selectedLead.name}
                        </h2>
                          ...
                          {selectedLead.destination && (
                            <div>
                              ...
                              <div className="mt-1 flex items-center space-x-2">
                                ...
                                <span className="text-sm text-gray-900">{getDestinationName(selectedLead.destination)}</span>
                              </div>
                            </div>
                          )}
